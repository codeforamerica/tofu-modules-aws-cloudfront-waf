...
| certificate_sans         | Additional ACM subject alternative names such as `["*.example.org"]` for hostnames beyond the primary FQDN. Duplicates of the primary name are ignored. Only used when `certificate_imported` is `false`.         | `list(string)` | `[]`              | no          |
